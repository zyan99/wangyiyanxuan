...
if($rows>0){
	...
	$num = $row['product_num']+$count;
	$sql = "UPDATE `cart` SET `product_num`=$num WHERE `product_id`=$id";
}else{
	$sql = "INSERT INTO `cart` (`product_id`,`product_img`,`product_name`,`product_num`,`product_price`) VALUES ('$id','$img','$name','$count','$price')";
}
